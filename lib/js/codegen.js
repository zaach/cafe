...
var codegens = exports.nodes = {
    ...
    'ParamDecl': function ParamDecl_codegen (lvl) {
        return this.children.map(function(node){print(node.nodeType);return node.toJS(lvl)}).join(', ');
    },
...
function genStmts (nodes, lvl) {
    return nodes.map(function(node){return node.blockgen()}).join('\n'+idt(lvl));
}
